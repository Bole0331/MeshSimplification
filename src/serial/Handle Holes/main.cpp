...
void update(int x, int except = -1) {
	order[x] += 1;
	for (map<int, int>::iterator i = next[x].begin(); i != next[x].end(); i++) {
		int a = i->first, b = x;
		if (a == except || a < 0) continue;
		if (a > b) swap(a, b);
...
void prepare() {
	for (int i = 0; i < v.size(); i++ ) 
		for (map<int, int>::iterator j = next[i].begin(); j != next[i].end(); j++ ) 
			// second condition, for hole correctness
			if (i < j->first || next[j->first].count(i) == 0) 
...
int main(int argc, char * argv[]){
	...
	// for hole correctness
	for (int i = 0; i < v.size(); i ++) reshape(i);
